...
const ZINE_URL = '/z/riverwild/hidden-rivers-under-the-city';
...
test('a zine renders semantic content from its document', async ({ page }) => {
	...
	await expect(
		page.getByRole('heading', { level: 1, name: 'Hidden Rivers Under the City' })
	).toBeVisible();
	await expect(
		page.getByRole('heading', {
			level: 2,
			name: 'A suspiciously specific field guide to storm drains'
		})
	).toBeVisible();
	await expect(page.getByRole('img', { name: /storm drain/i })).toBeVisible();
	await expect(page.getByRole('link', { name: 'Read visual essays at The Pudding' })).toBeVisible();
	await expect(page.getByText('By Riverwild Field Notes')).toBeVisible();
});
